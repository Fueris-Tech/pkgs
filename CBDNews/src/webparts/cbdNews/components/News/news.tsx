...
export const News: React.FunctionComponent<INews> = (props) => {
    ...
    const fetchnewsChoices = async () => {
        ...
        setCategories(p => ["Employee Communication", "Business Updates"]);
        setCategory(p => "Employee Communication");
        setLoading(false);
    }

    const fetchnewsByCategory = async (categoryVal: string) => {
        let val = categoryVal === "Employee Communication" ? "Employee News" : "CBD News";
        if (categoryVal) {
            setLoading(false);
            const sp = spfi().using(SPFx(props.context));
            const results: any = await sp.web.lists
                .getByTitle("News")
                .items.filter(`Category eq '${val}' and Status eq 'Approved'`)
                .top(20)
    ...
    return (
        ...
        <div className="font-cairo max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto pt-10 pb-16 news">
            <div className="text-4xl text-gray1 font-bold px-2 pt-8 pb-4">News & Announcements</div>
            {categories && news && (
                <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-between">
                        ...
                        <a className="w-fit font-bold text-base text-primary  flex-none flex gap-2 items-center" href={props.link} ><span>View All</span><Icon iconName="ChevronRight" /></a>
                    </div>
                    {loading && <div ref={sliderRef} className="keen-slider py-4">
                        {news?.map((n: any, index: number) => (
                            <div key={`slides_${index}`} className="keen-slider__slide h-1339 shadow-lg rounded-lg p-1" style={{ maxWidth: 330, minWidth: 330 }} >
                                <a href={`${props.detailLink}?NewsId=${n.Id}`}><img src={n.Thumbnail_x0020_Photo.Url} className="w-full h-1170 object-fill rounded-lg" /></a>
                                <p className="text-sm px-2 pt-4 pb-2 font-bold text-gray3">{dateFormat(n.Modified, "d mmmm yy")} </p>
                                <a className="text-lg px-2 block text-gray1 overflow-hidden" href={`${props.detailLink}?NewsId=${n.Id}`}>
