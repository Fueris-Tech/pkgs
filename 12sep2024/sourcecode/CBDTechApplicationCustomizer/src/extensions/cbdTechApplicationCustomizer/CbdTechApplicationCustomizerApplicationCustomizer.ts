...
export interface ICbdTechApplicationCustomizerApplicationCustomizerProperties {
  cssurl: string;
  headerSiteUrl: string;
}

export default class CbdTechApplicationCustomizerApplicationCustomizer
  extends BaseApplicationCustomizer<ICbdTechApplicationCustomizerApplicationCustomizerProperties> {

  private header: PlaceholderContent | undefined;
  //private footer: PlaceholderContent | undefined;

  private hidecss() {
    const headerElements: any = document.querySelectorAll('div[data-sp-feature-tag^="PageTitle web part"],#SuiteNavPlaceHolder, #SuiteNavWrapper, .sp-appBar, .ms-HubNav, #spSiteHeader'); // Replace with the actual header selector
    if (headerElements) {
