    ...
    const fetchNewsItem = async () => {
        ...
        if (params.get('NewsId')) {
            ...
            try {
                ...
                const sp = spfi().using(SPFx(props.context));
                const result = await sp.web.lists.getByTitle("CBDTechNews").items.select("*").getById(id)();
                setNewsItem(result);
            } catch (error) {
                setNewsItem(null);
            }
            ...
            {newsItem && <>
                <div className="w-full rounded-lg relative max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto" style={{ "height": "340px" }}>
                    ...
                    <div className="absolute bottom-140 left-140 z-10 text-4xl text-white text-bold ">
                        <p className="w-fit px-4 py-2 rounded-full border border-white text-sm text-white bg-transparent font-bold">{newsItem.Category}</p>
                ...
                <div className="max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto py-10">
                    <p className="text-sm py-2 font-bold text-gray3">{dateFormat(newsItem.PublishingDate, "d mmmm yy")}</p>
