...
export const Search: React.FunctionComponent<ISearch> = (props) => {
    ...
    const fetchNews = async (page: number = 1) => {
        ...
        const results: any = await sp.web.lists.getByTitle("CBDTechNews").getItemsByCAMLQuery(query);
                ...
                <div className="flex flex-col gap-8 py-10">
                    ...
                    <div className="bg-light h-full py-10" style={{ "minHeight": "440px" }}>
                        <div className="flex w-full items-center gap-2 py-4 max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto searcharea">
                            <div className="grow">
                                <SearchBox placeholder="Search News & Updates" styles={{
                        ...
                        <div className="flex flex-wrap gap-4 w-full max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto">
                            {news && news.map((n: any, index: number) => (
                                <div key={`search${index}`} className="bg-white h-1400 shadow-lg rounded-lg p-1 pb-8" style={{ maxWidth: 330, minWidth: 330 }}>
                                    <p className="pb-4">
                                        <a href={`${props.detailLink}?env=WebView&NewsId=${n.Id}`}><img src={n.Thumbnail.Url} className="w-full h-1220 object-cover rounded-lg" /></a>
                                    </p>
                                    <p className="w-fit px-4 py-2 mx-4 rounded-full border border-gray4 text-sm text-primary bg-white font-bold">{n.Category}</p>
                                    <p className="text-sm p-2 mx-4 font-bold text-gray3">{dateFormat(n.PublishingDate, "d mmmm yy")}</p>
