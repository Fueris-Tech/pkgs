                                        ...
                                        <p className="flex-none w-fit  px-4 py-4 flex gap-2 flex-wrap ">
                                            {
                                                bcmItems.filter((hhh: any) => hhh.Category === 'Link' && hhh.ParentId === it.Id).map((iit: any, iindex: number) => (
                                                    <a key={`Link_${index}_${iindex}`} data-interception="off" rel="noopener noreferrer" target={iit.IsNew ? "_blank" : "_self"} className="text-base text-primary font-bold bg-light rounded-full block px-4 py-2 text-nowrap hover:bg-primary hover:text-white" href={appendEnvToSharePointUrl(iit.LinkToPage, iit.IsNew)}>{iit.Title}</a>
                                                ))
                                            }</p>
