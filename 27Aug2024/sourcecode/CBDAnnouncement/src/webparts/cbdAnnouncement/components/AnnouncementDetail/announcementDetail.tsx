...
export const AnnouncementDetail: React.FunctionComponent<IAnnouncementDetail> = (props) => {

    const [breadcrumbItems, setBreadcrumbItems] = React.useState<IBreadcrumbItem[]>([]);
    const [announcementItem, setAnnouncementItem] = React.useState<any>(null);
    ...
    const fetchAnnouncementItem = async () => {
        const params = new URLSearchParams(window.location.search);
        if (params.get('AId')) {
            const id = parseInt(params.get('AId') || '');
            try {

                const sp = spfi().using(SPFx(props.context));
                const result = await sp.web.lists.getByTitle("CBDAnnouncement").items.select("*").getById(id)();
                setAnnouncementItem(result);
            } catch (error) {
            ...
            {announcementItem && <>
                <div className="w-full rounded-lg relative max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto" style={{ "height": "340px" }}>
                    ...
                    <div className="absolute top-340 left-140 z-10 text-4xl text-white text-bold ">
                        <p className="w-fit px-4 py-2 rounded-full border border-white text-sm text-white bg-transparent font-bold">{announcementItem.Title}</p>
                        <p className="text-4xl py-2 font-bold text-white max-w-mdmax" >{announcementItem.Summary}</p>
                    </div>
                    <div className="relative mix-blend-overlay w-full h-full rounded-lg overflow-hidden">
                        ...
                        <img src={announcementItem?.Thumbnail?.Url} alt={"Announcement"} className="absolute object-cover w-full h-full opacity-50" />
                    </div>
                </div>
                <div className="max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto py-10">
                    <p className="text-sm py-2 font-bold text-gray3">{dateFormat(announcementItem.Modified, "d mmmm yy")}</p>
                    <div dangerouslySetInnerHTML={{ __html: announcementItem.PageContent }} />
                </div>
            </>
            }
        </div>
