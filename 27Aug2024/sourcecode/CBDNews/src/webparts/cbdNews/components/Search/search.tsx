...
export const Search: React.FunctionComponent<ISearch> = (props) => {
    ...
    const fetchnewsChoices = async () => {
        setCategories(p => ["All", "Employee Communication", "Business Updates"]);
    }
    ...
    const generateCamlQuery = (searchVal: string, page: number) => {
        ...
        if (category !== 'All') {
            let c = category === "Employee Communication" ? "Employee News" : "CBD News";
            ...
            filterQuery = filterQuery
                ? `<And>
                    <Or>
                        <Eq>
                            <FieldRef Name='Category' />
                            <Value Type='Text'>CBD News</Value>
                        </Eq>
                        <Eq>
                            <FieldRef Name='Category' />
                            <Value Type='Text'>Employee News</Value>
                        </Eq>
                    </Or>
                    ${filterQuery}
                </And>`
        ...
        return `<View>
            <Query>
                ...
                <OrderBy>
                <FieldRef Name='Modified' Ascending='${sort !== 'Recent'}' />
                </OrderBy>
            </Query>
    ...
    const fetchNews = async (page: number = 1) => {
        ...
        const results: any = await sp.web.lists.getByTitle("News").getItemsByCAMLQuery(query);
        setNews(prevNews => page === 1 ? results : [...prevNews, ...results]);
        setLoading(false);
    ...
    const fetchNextPage = () => {
        fetchNews(Math.floor(news.length / props.count) + 1);
    };
    ...
    const handleScroll = () => {
        ...
        if (elem) {
            const lastItem = document.querySelector(".news:last-child");
            if (lastItem) {
        ...
        <div className="font-cairo m-auto max-w-xlmax news pt-8 pb-20">
            ...
            <div className="w-full rounded-lg relative font-cairo max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto m-auto " style={{ "height": "300px" }}>
                ...
                <div className="relative mix-blend-overlay w-full h-full rounded-lg overflow-hidden">
                    ...
                    <img src={props.bannerImageUrl} alt={"News"} className="absolute object-cover w-full h-full opacity-50" />
                </div>
            </div>
            {categories && (
                <div className="flex flex-col gap-8 py-10">
                    ...
                    <div className="bg-light h-full py-10" style={{ "minHeight": "440px" }}>
                        <div className="flex w-full items-center gap-2 py-4 max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto searcharea">
                            <div className="grow">
                                <SearchBox placeholder="Search News & Announcements" styles={{
                                    icon: { color: 'text-blue-500' }
                                }} onChange={(e) => setSearchVal(e?.target.value || '')} underlined={true} className="bg-transparent max-w-smmax" />
                            </div>
                        ...
                        <div className="flex flex-wrap gap-4 w-full max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto">
                            {news && news.map((n: any, index: number) => (
                                <div key={`search${index}`} className="bg-white h-1400 shadow-lg rounded-lg p-1 pb-8" style={{ maxWidth: 330, minWidth: 330 }}>
                                    <p className="pb-4">
                                        <a href={`${props.detailLink}?env=WebView&NewsId=${n.Id}`}><img src={n.Thumbnail_x0020_Photo.Url} className="w-full h-1220 object-cover rounded-lg" /></a>
                                    </p>
                                    <p className="w-fit px-4 py-2 mx-4 rounded-full border border-gray4 text-sm text-primary bg-white font-bold">{n.Category === "Employee News" ? "Employee Communication" : "Business Updates"}</p>
                                    <p className="text-sm p-2 mx-4 font-bold text-gray3">{dateFormat(n.Modified, "d mmmm yy")}</p>
                                    <a className="text-lg px-2 mx-4 block text-gray1 overflow-hidden" title={n.Summary} href={`${props.detailLink}?env=WebView&NewsId=${n.Id}`}>
                                        {n.Summary ? (n.Summary.length > 50 ? `${n.Summary.substring(0, 50)}...` : n.Summary || '') : (n.Title.length > 50 ? `${n.Title.substring(0, 50)}...` : n.Title || '')}
                                    </a>
                                </div>
                            ))}
                        </div>
