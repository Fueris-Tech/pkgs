...
export const PhotoGallery: React.FunctionComponent<IPhotoGallery> = (props) => {
    ...
    return (
        <div className="font-cairo max-w-xmdmax desktop:max-w-xlgmax large:max-w-lgmax  mx-auto pt-10 pb-8 photogallery">
                        ...
                        {slides?.map((slide: any, index: number) => (
                            <div key={`slides_${index}`} className="keen-slider__slide h-340 " style={{ maxWidth: 225, minWidth: 225 }} >
                                {slides.length - 1 === index && <div className="absolute w-full h-full grid place-items-center bg-black bg-opacity-50">
                                    <a className="flex w-fit gap-2 " href={`${props.detaillink}?IId=${gallery.Id}`}><span className="text-base text-white font-bold">See All Photos</span><Icon iconName="ChevronRight" className="text-base text-white font-bold" /> </a></div>}
